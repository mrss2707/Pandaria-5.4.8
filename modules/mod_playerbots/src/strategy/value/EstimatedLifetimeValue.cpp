...
float EstimatedGroupDpsValue::Calculate()
{
    ...
    for (Player* player : groupPlayer)
    {
        ...
        if (PlayerBotSpec::IsTank(player))
            roleMultiplier = 0.3f;
        else if (PlayerBotSpec::IsHeal(player))
            roleMultiplier = 0.1f;
        else
            roleMultiplier = 1.0f;
...
float EstimatedGroupDpsValue::GetBasicGs(uint32 level)
{
    ...
    if (level <= 8)
    {
        basic_gs = 5;// PlayerbotFactory::CalcMixedGearScore(level + 5, ITEM_QUALITY_NORMAL);
    }
    else if (level <= 15)
    {
        basic_gs = 30;// PlayerbotFactory::CalcMixedGearScore(level + 5, ITEM_QUALITY_UNCOMMON);
    }
    else if (level <= 60)
    {
        basic_gs = 100;// PlayerbotFactory::CalcMixedGearScore(level + 5, ITEM_QUALITY_RARE);
    }
    else if (level <= 70)
    {
        basic_gs = 200;// PlayerbotFactory::CalcMixedGearScore(85 + (level - 60) * 3, ITEM_QUALITY_RARE);
    }
    else
    {
        basic_gs = 400;// PlayerbotFactory::CalcMixedGearScore(155 + (level - 70) * 4, ITEM_QUALITY_RARE);
    }
    return basic_gs;
}