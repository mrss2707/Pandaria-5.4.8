...
class instance_stratholme : public InstanceMapScript
{
    public:
        ...
        struct instance_stratholme_InstanceMapScript : public InstanceScript
        {
            ...
            std::list<ObjectGuid> crystalsGUID;
            std::list<ObjectGuid> abomnationGUID;
            std::list<ObjectGuid> qBrotherhoodGUID;
            EventMap events;
            ...
            void Initialize() override
            {
                ...
                serviceEntranceGUID = ObjectGuid::Empty;
                gauntletGate1GUID = ObjectGuid::Empty;
                ziggurat1GUID = ObjectGuid::Empty;
                ziggurat2GUID = ObjectGuid::Empty;
                ziggurat3GUID = ObjectGuid::Empty;
                ziggurat4GUID = ObjectGuid::Empty;
                ziggurat5GUID = ObjectGuid::Empty;
                portGauntletGUID = ObjectGuid::Empty;
                portSlaugtherGUID = ObjectGuid::Empty;
                portElderGUID = ObjectGuid::Empty;

                baronGUID = ObjectGuid::Empty;
                ysidaTriggerGUID = ObjectGuid::Empty;
                crystalsGUID.clear();
                abomnationGUID.clear();
                qBrotherhoodGUID.clear();
            }
            ...
            void OnCreatureCreate(Creature* creature) override
            {
                switch (creature->GetEntry())
                {
                    ...
                    case NPC_YSIDA_TRIGGER:
                        ...
                        break;
                    case NPC_CRYSTAL:
                        crystalsGUID.push_back(creature->GetGUID());
                        break;
                    case NPC_ABOM_BILE:
                    case NPC_ABOM_VENOM:
                        abomnationGUID.push_back(creature->GetGUID());
                        break;
                    case NPC_COMMANDER_ELIGOR:
                    case NPC_PACKMASTER_STONEBRUISER:
                    case NPC_WILHELM:
                        if (creature->GetPositionY() > -3364.41f)
                        {
                            qBrotherhoodGUID.push_back(creature->GetGUID());
            ...
            void OnUnitDeath(Unit* unit) override
            {
                if (unit->ToCreature())
                {
                    ...
                    {
                        case NPC_BALNAZZAR:
                            for (auto&& qBrotherhood : qBrotherhoodGUID)
                                if (Creature* questgiver = instance->GetCreature(qBrotherhood))
                                    if (questgiver->AI())
                                        questgiver->AI()->DoAction(ACTION_BALNAZZAR);
                            break;
                        case NPC_ABOM_BILE:
                        case NPC_ABOM_VENOM:
                            if (std::find(abomnationGUID.begin(), abomnationGUID.end(), unit->GetGUID()) != abomnationGUID.end())
                                abomnationGUID.erase(std::find(abomnationGUID.begin(), abomnationGUID.end(), unit->GetGUID()));

                            if (abomnationGUID.empty())
                                HandleGameObject(GetGuidData(GO_ZIGGURAT4), true);
                            break;
                    }
                }
            }
            ...
            void OnCreatureRemove(Creature* creature) override
            {
                ...
                {
                    case NPC_CRYSTAL:
                        if (std::find(crystalsGUID.begin(), crystalsGUID.end(), creature->GetGUID()) != crystalsGUID.end())
                            crystalsGUID.erase(std::find(crystalsGUID.begin(), crystalsGUID.end(), creature->GetGUID()));
                        break;
                    default:
                        break;
                }
            ...
            void SetData(uint32 type, uint32 data) override
            {
                switch (type)
                {
                    ...
                    case TYPE_RAMSTEIN:
                        ...
                        if (data == DONE)
                        {
                            events.ScheduleEvent(EVENT_SLAUGHTER_SQUARE, 10000);
