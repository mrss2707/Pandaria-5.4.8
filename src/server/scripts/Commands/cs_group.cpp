...
public:
    ...
    static bool HandleGroupCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!group)
        {
            handler->PSendSysMessage("Player not found or is not in a group");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage("LFG State: %s (%u)", lfg::GetStateString(state).c_str(), uint8(state));
            handler->PSendSysMessage("LFG Dungeon: %u (as entry: %u)", sLFGMgr->GetDungeon(group->GetGUID(), true), sLFGMgr->GetDungeon(group->GetGUID()));
        }
