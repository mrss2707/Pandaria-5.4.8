...
public:
    ...
    */
    ...
    static bool HandleGoCreatureCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (Creature* creature = sObjectAccessor->GetCreature(*player, MAKE_NEW_GUID(guid, id, HighGuid::Unit)))
        {
    ...
    static bool HandleGoTriggerCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            handler->PSendSysMessage(LANG_INVALID_TARGET_COORD, at->Pos.X, at->Pos.Y, at->ContinentID);
            handler->SetSentErrorMessage(true);
            return false;
        }
