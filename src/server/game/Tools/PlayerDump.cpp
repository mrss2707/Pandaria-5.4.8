...
static DumpTable dumpTables[DUMP_TABLE_COUNT] =
{
    ...
    { "character_achievement",            DTT_ACHIEVEMENT },
    ...
    { "character_declinedname",           DTT_CHARACTER_DECL },
    ...
    { "character_research_digsites",      DTT_CHAR_TABLE },
    { "character_research_history",       DTT_CHAR_TABLE },
    { "character_research_projects",      DTT_CHAR_TABLE },
    { "character_skills",                 DTT_CHAR_TABLE },
    { "character_spell",                  DTT_SPELL      },
    { "character_spell_cooldown",         DTT_CHAR_TABLE },
    { "character_talent",                 DTT_CHAR_TABLE },
    { "character_void_storage",           DTT_VOID_STORAGE },
...
bool findnth(std::string const& str, int n, std::string::size_type &s, std::string::size_type &e)
{
    s = str.find("VALUES ('")+9;
...
uint32 getnthUInt(std::string const& str, int n)
{
    std::string::size_type s, e;
    if (!findnth(str, n, s, e))
        return 0;
    ...
    char* end;
    uint32 ret = strtoul(str.c_str() + s, &end, 10);
    ASSERT(end == str.c_str() + e);
    return ret;
}
...
bool PlayerDumpWriter::DumpTable(std::string& dump, uint32 guid, char const*tableFrom, char const*tableTo, DumpTableType type)
{
    ...
    switch (type)
    {
        ...
        case DTT_VOID_STORAGE: fieldname = "playerGuid";             break;
...
DumpReturn PlayerDumpReader::LoadDump(std::string const& file, uint32 account, std::string name, ObjectGuid::LowType guid, CharacterDatabaseTransaction trans)
{
    ...
    std::map<uint32, uint32> itemsVoid;
    std::set<uint32> classSpells;
    std::set<uint32> spells;
    std::set<uint32> accountSpells;
    std::set<uint32> existsAccountSpells;
    char buf[32000] = "";
    ...
    if (result)
    {
        do
        {
            existsAccountSpells.insert((*result)[0].GetUInt32());
        } while (result->NextRow());
    }
    ...
    while (!feof(fin))
    {
        ...
        */
        ...
        switch (type)
        {
            ...
            case DTT_CHARACTER_DECL:
            {
                // Skip character_declinedname when loading dump with custom name
                if (!keepOriginalName)
                    continue;

                if (!changenth(line, 1, newguid))           // character_declinedname.guid update
                    ROLLBACK(DUMP_FILE_BROKEN);

                declinedName = new DeclinedName();
                for (uint8 i = 0; i < MAX_DECLINED_NAME_CASES; ++i)
                    declinedName->name[i] = getnth(line, 2 + i);
            ...
            case DTT_INVENTORY:
            {
                ...
                if (!changeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>().Generate(), true))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_inventory.bag update
                if (!changeGuid(line, 4, items, sObjectMgr->GetGenerator<HighGuid::Item>().Generate()))
            ...
            case DTT_MAIL_ITEM:                             // mail_items
            {
                ...
                if (!changeGuid(line, 2, items, sObjectMgr->GetGenerator<HighGuid::Item>().Generate()))
            ...
            case DTT_ITEM:
            {
                ...
                if (!changeGuid(line, 1, items, sObjectMgr->GetGenerator<HighGuid::Item>().Generate()))
            ...
            case DTT_SPELL:
            {
                uint32 spellId = getnthUInt(line, 2);
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
                if (!spellInfo)
                    continue;
                if (spellInfo->IsAccountWide())
                {
                    if (existsAccountSpells.find(spellId) == existsAccountSpells.end())
                        accountSpells.insert(spellId);
                    continue;
                }
                if (classSpells.find(spellId) != classSpells.end())
                    continue;
                if (!changenth(line, 1, newguid))
                    ROLLBACK(DUMP_FILE_BROKEN);
                break;
            }
            case DTT_ACHIEVEMENT:
            {
                uint32 achievementId = getnthUInt(line, 2);
            ...
            case DTT_VOID_STORAGE:
            {
                if (!changeGuid(line, 1, itemsVoid, sObjectMgr->_voidItemId))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_void_storage.itemId
                if (!changenth(line, 2, newguid))
                    ROLLBACK(DUMP_FILE_BROKEN);             // character_void_storage.playerGuid
                break;
            }
            default:
    ...
    sObjectMgr->_voidItemId += itemsVoid.size();
