...
{
    public:
        explicit ActivePoolData(Map* owner);
        ~ActivePoolData();
...
class PoolGroup
{
    ...
    public:
        ...
        void Despawn1Object(ActivePoolData& spawns, ObjectGuid::LowType guid);
        void SpawnObject(ActivePoolData& spawns, uint32 limit, uint32 triggerFrom);

        void Spawn1Object(ActivePoolData& spawns, PoolObject* obj);
        void ReSpawn1Object(ActivePoolData& spawns, PoolObject* obj);
...
class PoolMgr
{
    ...
    public:
        ...
        template<typename T>
        bool IsSpawnedObject(ActivePoolData& activePoolData, uint32 db_guid_or_pool_id) const { return activePoolData.IsActiveObject<T>(db_guid_or_pool_id); }

        bool IsEmpty(uint32 pool_id) const;
        bool CheckPool(uint32 pool_id) const;

        void SpawnPool(ActivePoolData& activePoolData, uint32 pool_id);
        void DespawnPool(ActivePoolData& activePoolData, uint32 pool_id);
    ...
    private:
        template<typename T>
        void SpawnPool(ActivePoolData& activePoolData, uint32 pool_id, uint32 db_guid_or_pool_id);
