...
{
    CharacterDatabasePreparedStatement* delStmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_QUEST_POOL_SAVE);
    delStmt->setUInt32(0, pool.poolId);
    trans->Append(delStmt);

    for (uint32 questId : pool.activeQuests)
    {
        CharacterDatabasePreparedStatement* insStmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_QUEST_POOL_SAVE);
...
void QuestPoolMgr::LoadFromDB()
{
    ...
    // load template data from world DB
    {
        ...
        if (!result)
        {
            TC_LOG_INFO("server.loading", ">> Loaded 0 quest pools. DB table `pool_quest` is empty.");
            return;
        }
    ...
    for (auto const& pair : lookup)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `pool_template` contains quest pool %u requesting %u spawns, but only has %lu members. Requested spawns reduced.", pool.poolId, pool.numActive, pool.members.size());
        ...
        if (!doRegenerate)
        {
            ...
            for (size_t i = pool.members.size(); (i--);)
            {
                ...
                if (member.empty())
                {
                    TC_LOG_ERROR("sql.sql", "Table `pool_quest` contains no entries at index %lu for quest pool %u. Index removed.", i, pool.poolId);
                ...
                for (auto it = member.begin(); (++it) != member.end();)
                {
                    ...
                    if (status != otherStatus)
                        TC_LOG_WARN("sql.sql", "Table `pool_quest_save` %s quest %u (in pool %u, index %lu) saved, but its index is%s active (because quest %u is%s in the table). Set quest %u to %sactive.", (status ? "does not have" : "has"), *it, pool.poolId, i, (status ? "" : " not"), member[0], (status ? "" : " not"), *it, (status ? "" : "in"));
                    if (otherStatus)
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu daily, %lu weekly and %lu monthly quest pools in %u ms", _dailyPools.size(), _weeklyPools.size(), _monthlyPools.size(), GetMSTimeDiffToNow(oldMSTime));
}
