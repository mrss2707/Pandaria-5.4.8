...
enum
{
    WORLD_STATE_THE_PERFECT_STORM   = 5720,
    WORLD_STATE_STORM_GLORY         = 5719,

    EVENT_START_BATTLE_EY           = 13180, // 214 - Flurry
};
...
void BattlegroundEY::StartingEventOpenDoors()
{
    ...
    // Achievement: Flurry
    StartCriteria(CRITERIA_START_TYPE_EVENT, EVENT_START_BATTLE_EY);
}
...
void BattlegroundEY::UpdateTeamScore(uint32 team)
{
    ...
    if (score >= BG_EY_MAX_TEAM_SCORE)
    {
        ...
        if (!GetTeamScore(team == TEAM_ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE))
            GetBgMap()->SetWorldState(WORLD_STATE_THE_PERFECT_STORM, 1);
...
void BattlegroundEY::UpdatePointsCount(uint32 Team)
{
    ...
    bool gloryCondition = m_TeamPointsCount[TEAM_ALLIANCE] == 4 || m_TeamPointsCount[TEAM_HORDE] == 4;
    GetBgMap()->SetWorldState(WORLD_STATE_STORM_GLORY, gloryCondition ? 1 : 0);
}
...
{
        // doors
    if (!AddObject(BG_EY_OBJECT_DOOR_A, BG_OBJECT_A_DOOR_EY_ENTRY, 2526.87f, 1596.9f, 1243.72f, -3.12414f, -0.173642f, -0.001515f, 0.98477f, -0.008594f, RESPAWN_IMMEDIATELY)
        || !AddObject(BG_EY_OBJECT_DOOR_H, BG_OBJECT_H_DOOR_EY_ENTRY, 1793.0f, 1539.22f, 1248.67f, 3.14159f, 0.173648f, 0, 0.984808f, 0, RESPAWN_IMMEDIATELY)
        // banners (alliance)
