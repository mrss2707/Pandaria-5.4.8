...
SmartAI::SmartAI(Creature* creature) : CreatureAI(creature),
...
SmartVehicleAI::SmartVehicleAI(Creature* c) : SmartAI(c), VehicleAIBase(c)
{
    mSmartVehicle = true;
}

void SmartVehicleAI::UpdateAI(uint32 diff)
{
    SmartAI::UpdateAI(diff);

    // Vehicle might have already been despawned by SmartAI scripts
    if (me->IsInWorld() && me->GetVehicleKit() == m_vehicle)
        VehicleAIBase::UpdateAI(diff);
}

int SmartVehicleAI::Permissible(Creature const* creature)
{
    if (creature->GetAIName() == "SmartVehicleAI")
        return PERMIT_BASE_SPECIAL;
    return PERMIT_BASE_NO;
}

void SmartVehicleAI::Reset()
{
    VehicleAIBase::Reset();
    SmartAI::Reset();
}

void SmartVehicleAI::MoveInLineOfSight(Unit* who)
{
    VehicleAIBase::MoveInLineOfSight(who);
}

void SmartVehicleAI::AttackStart(Unit* who)
{
    VehicleAIBase::AttackStart(who);
}

void SmartVehicleAI::OnCharmed(bool apply)
{
    VehicleAIBase::OnCharmed(apply);
...
bool SmartGameObjectAI::OnReportUse(Player* player)
{
    ...
    GetScript()->ProcessEventsFor(SMART_EVENT_GOSSIP_HELLO, player, 1, 0, false, nullptr, me); // back compatibility
...
{
    GetScript()->ProcessEventsFor(SMART_EVENT_ACCEPTED_QUEST, player, quest->GetQuestId(), 0, false, NULL, me);
    return false;
}

// Called when a player selects a quest reward.
bool SmartGameObjectAI::OnQuestReward(Player* player, Quest const* quest, uint32 opt)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_REWARD_QUEST, player, quest->GetQuestId(), opt, false, NULL, me);
    return false;
}
...
{
    GetScript()->ProcessEventsFor(SMART_EVENT_DEATH, player, eventId, 0, false, NULL, me);
}
