...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex)
{
    ...
    TC_LOG_DEBUG("spells", "Spell: Player %u has unlearned spell %u from NpcGUID: %u", player->GetGUID().GetCounter(), spellToUnlearn, m_caster->GetGUIDLow());
}
    ...
    if (gameObjTarget)
    {
        ...
        if (!gameObjTarget->isSpawned() && !player->IsGameMaster())
        {
            TC_LOG_ERROR("spells", "Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!",
                            player->GetName().c_str(), player->GetGUID().GetCounter(), gameObjTarget->GetEntry(), gameObjTarget->GetGUIDLow());
            return;
        }
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    TC_LOG_DEBUG("spells", "Spell: Player %u has learned spell %u from NpcGUID=%u", player->GetGUID().GetCounter(), spellToLearn, m_caster->GetGUIDLow());
}
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), gameobject_id, map,
    ...
    if (uint32 linkedEntry = pGameObj->GetGOInfo()->GetLinkedGameObjectEntry())
    {
        ...
        if (linkedGO->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), linkedEntry, map,
            m_caster->GetPhaseMask(), x, y, z, target->GetOrientation(), { }, 100, GO_STATE_READY))
        {
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), gameobject_id,
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
    ...
    Map* map = m_caster->GetMap();
    if (!go->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), go_id, map,
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    if (!pGameObj->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), name_id, cMap,
    ...
    {
        GameObject* linkedGO = new GameObject;
        if (linkedGO->Create(sObjectMgr->GenerateLowGuid(HighGuid::GameObject), linkedEntry, cMap,
            m_caster->GetPhaseMask(), fx, fy, fz, m_caster->GetOrientation(), { }, 100, GO_STATE_READY))
        {
            linkedGO->AddToTransportIfNeeded(m_caster->GetTransport());
