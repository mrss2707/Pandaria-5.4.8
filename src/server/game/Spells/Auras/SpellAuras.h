...
enum SpellControlType
{
    CONTROL_TYPE_NONE               = 0,
    CONTROL_TYPE_POSSESS            = 1,
    CONTROL_TYPE_CONFUSE            = 2,
    CONTROL_TYPE_CHARM              = 3,
    CONTROL_TYPE_FEAR               = 4,
    CONTROL_TYPE_LOST_CONTROL       = 5,
    CONTROL_TYPE_PACIFY             = 6,
    CONTROL_TYPE_ROOT               = 7,
    CONTROL_TYPE_SILENCE            = 8,
    CONTROL_TYPE_PACIFY_AND_SILENCE = 9,
    CONTROL_TYPE_DISARM             = 10,
    CONTROL_TYPE_INTERRUPT          = 11,
    CONTROL_TYPE_STUN               = 12,
};

struct ControlInfo
{
    ControlInfo() : mechanic(MECHANIC_NONE), controlType(CONTROL_TYPE_NONE), effectIndex(0) { }
    ControlInfo(uint32 _mechanic, uint32 _controlType, uint8 _effectIndex) : mechanic(_mechanic), controlType(_controlType), effectIndex(_effectIndex) { }
...
struct AuraData
{
    ...
    float Amount[MAX_SPELL_EFFECTS];
    float Crit[MAX_SPELL_EFFECTS];
    int32 PeriodicTimer[MAX_SPELL_EFFECTS];
    uint32 TickNumber[MAX_SPELL_EFFECTS];
};
...
class Aura
{
    ...
    public:
        ...
        explicit Aura(SpellInfo const* spellproto, WorldObject* owner, Unit* caster, Item* castItem, ObjectGuid casterGUID, uint32 effMask, int32* baseAmount);
        ...
        void RemoveEffect(uint8 effIndex);
        ...
        int32 GetCalculatedMaxDuration() const { return m_calculatedMaxDuration; }
        ...
        uint8 GetMaxStackAmount() const { return m_maxStackAmount; }
        ...
        bool CanBeSentToClient(Unit* target) const;

        void BindToCaster() { m_isBoundToCaster = true; }
        void UnbindFromCaster() { m_isBoundToCaster = false; }
        bool IsBoundToCaster() const { return m_isBoundToCaster; }
        ...
        void CallScriptEffectAfterCalcAmountHandlers(AuraEffect const* aurEff, float& amount, bool& canBeRecalculated);
        void CallScriptEffectCalcPeriodicHandlers(AuraEffect const* aurEff, bool & isPeriodic, int32 & amplitude);
        void CallScriptEffectCalcTickAmountHandlers(AuraEffect const* eff, AuraApplication* app, int32& damage);
        void CallScriptEffectCalcSpellModHandlers(AuraEffect const* aurEff, SpellModifier* & spellMod);
        void CallScriptEffectAbsorbHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo & dmgInfo, uint32 & absorbAmount, bool & defaultPrevented);
        void CallScriptEffectAbsorbHealHandler(AuraEffect* aurEff, AuraApplication const* aurApp, uint32& heal, uint32& absorb, bool& defaultPrevented);
        ...
        void SaveParameters(AuraData& data);
        void LoadParameters(AuraData const& data);

        SpellPowerEntry const* GetSpellPowerEntry() const { return m_powerEntry; }
    ...
    protected:
        ...
        int32 m_maxDuration;                                // Max aura duration for current moment
        int32 m_calculatedMaxDuration;                      // Max aura duration calculated at aura creation
        int32 m_duration;                                   // Current time
        int32 m_powerTakeTimer;                             // Timer for power per sec calcultion
        ...
        bool m_isBoundToCaster:1;

        SpellPowerEntry const* m_powerEntry;
        bool m_hasPeriodicDamage = false;
